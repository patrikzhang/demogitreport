function gwtsamples(){
  ...
  $stats && $stats({moduleName:'gwtsamples', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'bootstrap', millis:(new Date).getTime(), type:'begin'});
  ...
  function maybeStartModule(){
    if (scriptsDone && loadDone) {
      var iframe = $doc_0.getElementById('gwtsamples');
      ...
      gwtsamples = null;
      frameWnd.gwtOnLoad(onLoadErrorFunc, 'gwtsamples', base, softPermutationId);
      $stats && $stats({moduleName:'gwtsamples', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'moduleStartup', millis:(new Date).getTime(), type:'end'});
    }
  }
  ...
  function computeScriptBase(){
    ...
    function tryNocacheJsTag(){
      ...
      for (var i = 0; i < scriptTags.length; ++i) {
        if (scriptTags[i].src.indexOf('gwtsamples.nocache.js') != -1) {
    ...
    function tryMarkerScript(){
      ...
      if (typeof isBodyLoaded == 'undefined' || !isBodyLoaded()) {
        var markerId = '__gwt_marker_gwtsamples';
        var markerScript;
  ...
  function processMetas(){
    ...
    for (var i = 0, n = metas.length; i < n; ++i) {
      ...
      if (name_0) {
        name_0 = name_0.replace('gwtsamples::', '');
        if (name_0.indexOf('::') >= 0) {
          continue;
        }
  ...
  function maybeInjectFrame(){
    if (!frameInjected) {
      ...
      iframe.id = 'gwtsamples';
      iframe.style.cssText = 'position:absolute;width:0;height:0;border:none';
      iframe.tabIndex = -1;
      $doc_0.body.appendChild(iframe);
      $stats && $stats({moduleName:'gwtsamples', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'moduleStartup', millis:(new Date).getTime(), type:'moduleRequested'});
  ...
  gwtsamples.onScriptLoad = function(){
    if (frameInjected) {
      loadDone = true;
      maybeStartModule();
    }
  ...
  ;
  gwtsamples.onInjectionDone = function(){
    scriptsDone = true;
    $stats && $stats({moduleName:'gwtsamples', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'loadExternalRefs', millis:(new Date).getTime(), type:'end'});
    maybeStartModule();
  }
  ;
  processMetas();
  ...
  var initialHtml;
  if (isHostedMode()) {
    if ($wnd_0.external && ($wnd_0.external.initModule && $wnd_0.external.initModule('gwtsamples'))) {
      $wnd_0.location.reload();
      return;
    }
    initialHtml = 'hosted.html?gwtsamples';
    strongName = '';
  }
  $stats && $stats({moduleName:'gwtsamples', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'bootstrap', millis:(new Date).getTime(), type:'selectingPermutation'});
  if (!isHostedMode()) {
    try {
      unflattenKeylistIntoAnswers(['opera'], '15F2286F3638401F359D9A27BEA6D482');
      unflattenKeylistIntoAnswers(['ie8'], '6BB5ABB91EDA22E6E0558B742E47BE77');
      unflattenKeylistIntoAnswers(['ie6'], 'B1D4F79117F5AC0F00AE3F8F4E008DEF');
      unflattenKeylistIntoAnswers(['safari'], 'B8B6CDD77DF61421E94A8DC86D6602B7');
      unflattenKeylistIntoAnswers(['gecko1_8'], 'C00953FCFA3175BA6C50D1486A13D9CD');
      unflattenKeylistIntoAnswers(['ie9'], 'C7D9BAB2AC1C09418C73EA288090AB6F');
  ...
  , 50);
  $stats && $stats({moduleName:'gwtsamples', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'bootstrap', millis:(new Date).getTime(), type:'end'});
  $stats && $stats({moduleName:'gwtsamples', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'loadExternalRefs', millis:(new Date).getTime(), type:'begin'});
  ...
  $doc_0.write('<script defer="defer">gwtsamples.onInjectionDone(\'gwtsamples\')<\/script>');
}
